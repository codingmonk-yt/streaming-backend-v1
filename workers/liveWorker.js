...
const { ExcludeLiveCategories } = require('../src/util/excludeCategories');
...
// Redis connection with BullMQ-required setting
...
const worker = new Worker(
  ...
  async (job) => {
    ...
    // Get credentials from provider's endpoint (adjust POST if needed)
    ...
    const filtered = liveStreams.filter(stream => {
      if (stream.category_id && ExcludeLiveCategories.includes(String(stream.category_id))) return false;
      if (
        stream.category_ids &&
        stream.category_ids.some(cid => ExcludeLiveCategories.includes(String(cid)))
      ) return false;
      return true;
    });

    // Upsert into database (Do NOT save username, password, dns)
